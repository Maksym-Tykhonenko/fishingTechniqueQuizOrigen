...
import {spinningFishing} from '../../data/spinningFishing';
import {spinningFishingArtikle} from '../../data/spinningFishingArtikle';
import AsyncStorage from '@react-native-async-storage/async-storage';

const SecondLvlScreen = ({navigation}) => {
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem(`SecondLvlScreen`, jsonData);
  ...
  const handleAnswer = selectedAnswer => {
    const currentQuestion = spinningFishing[currentQuestionIndex];
    ...
    if (currentQuestionIndex === spinningFishing.length - 1) {
      setIsPaused(true); // Зупиняємо таймер
      if (newCorrectAnswerCount === spinningFishing.length) {
        // Відображаємо модалку перемоги
        setModalIsCompliteLvl(true);
        setComplite2Level(true);
      } else {
          ...
          <ScrollView>
            <View style={{alignItems: 'center'}}>
              ...
              <ModalWithText
                ...
                text={spinningFishingArtikle.article}
...
export default SecondLvlScreen;
