...
//import {flyFishing} from '../../data/floatFishing';
import {flyFishing} from '../../data/flyFishing';
import {flyFishingArtikle} from '../../data/flyFishingArtikle';
import AsyncStorage from '@react-native-async-storage/async-storage';

const FirdLvlScreen = ({navigation}) => {
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem(`FirdLvlScreen`, jsonData);
  ...
  const handleAnswer = selectedAnswer => {
    const currentQuestion = flyFishing[currentQuestionIndex];
    ...
    if (currentQuestionIndex === flyFishing.length - 1) {
      setIsPaused(true); // Зупиняємо таймер
      if (newCorrectAnswerCount === flyFishing.length) {
        // Відображаємо модалку перемоги
        setModalIsCompliteLvl(true);
        setComplite3Level(true);
      } else {
          ...
          <ScrollView>
            <View style={{alignItems: 'center'}}>
              ...
              <ModalWithText
                ...
                text={flyFishingArtikle.article}
                modalState={modalIsCompliteLvl}
                goToo="FoureLvlScreen"
